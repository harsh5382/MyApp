...
export default function EditMemberScreen() {
  ...
  const { memberId, memberData: memberDataParam } = useLocalSearchParams();
  ...
  useEffect(() => {
    if (memberDataParam) {
      try {
        const parsedData = JSON.parse(memberDataParam as string);
  ...
  const handleSave = async () => {
    ...
    setIsLoading(true);
    try {
      await updateDoc(doc(db, "familyMembers", memberId as string), {
        ...memberData,
        updatedAt: new Date(),
      });
  ...
  return (
    <View style={styles.container}>
      <LinearGradient colors={["#121212", "#000000"]} style={styles.header}>
        ...
        <ThemedText style={styles.headerTitle}>Edit Member</ThemedText>
